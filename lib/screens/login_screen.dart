...
class _LoginScreenState extends State<LoginScreen> {
  ...
  final ApiService _apiService = ApiService('http://18.216.40.254:8080'); // Replace with your API endpoint
      ...
      try {
        ...
        if (accountId != null) {
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => HomeScreen(
                numeroConta: accountId.toString(),
              ),
            ),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Falha no login. Verifique suas credenciais e tente novamente.'),
            ),
          );
        }
      } catch (e) {
